...
function Slice(props) {
  if (process.env.GATSBY_SLICES) {
    ...
    const slicesContext = (0, _react.useContext)(_context.SlicesContext); // validate props
    ...
    if (slicesContext.renderEnvironment === `server`) {
      ...
      } catch {// don't need to handle it, we will just skip the additional context message if we fail to generate it
      }
...
class SlicePropsError extends Error {
  constructor(inBrowser, sliceName, propErrors, renderedByLocation) {
    ...
    if (inBrowser) {
      ...
      const fullStack = _react.default.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactDebugCurrentFrame.getCurrentStack(); // remove the first line of the stack trace
    ...
    if (renderedByLocation) {
      this.forcedLocation = { ...renderedByLocation,
        functionName: `Slice`
      };
    }
  }
